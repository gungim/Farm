...
func update(cooking: Cooking):
	if cooking:
		var recipe: Recipe = cooking.recipe
		$VBoxContainer/RecipeIcon/TextureRect.texture = recipe.icon
		$VBoxContainer/RecipeName.text = recipe.display_name

		remaining_time = recipe.time + cooking.start_time - Time.get_unix_time_from_system()
		if remaining_time <= 0:
...
func _on_timer_timeout():
	remaining_time -= 1
	time_label.text = GlobalEvents.format_time(int(remaining_time))
